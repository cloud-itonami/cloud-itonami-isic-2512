(ns metaltankmfg.operation-test
  "Smoke tests for the compiled MetalTankOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `metaltankmfg.governor-contract-test`; the Store contract in
  `metaltankmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "MetalTankOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "weld-001" :severity :moderate :description "weld-fume extraction hood underperforming near welding station"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
