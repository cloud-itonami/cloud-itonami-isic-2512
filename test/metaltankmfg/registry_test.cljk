(ns metaltankmfg.registry-test
  (:require [clojure.test :refer [deftest is]]
            [metaltankmfg.registry :as r]))
...
(deftest small-shipment-within-weight-does-not-exceed
  (is (false? (r/shipment-weight-exceeded?
               {:weight-kg 25000.0 :shipped-weight-kg 5000.0} 3000.0))))

(deftest shipment-that-pushes-past-weight-exceeds
  (is (true? (r/shipment-weight-exceeded?
              {:weight-kg 8000.0 :shipped-weight-kg 7600.0} 1000.0))))

(deftest shipment-exactly-at-weight-does-not-exceed
  (is (false? (r/shipment-weight-exceeded?
               {:weight-kg 8000.0 :shipped-weight-kg 7600.0} 400.0))
...
(deftest history-is-append-only
  (let [c1 (r/register-maintenance "mnt-1" "weld-001" 0)
        hist (r/append [] c1)
        c2 (r/register-maintenance "mnt-2" "weld-001" 1)
