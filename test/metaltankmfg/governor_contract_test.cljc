(ns metaltankmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control the welding line or pressure-
  testing/forming line directly... is NOT a pressure-vessel-
  certification authority... does NOT authorize or execute
  maintenance/welding-line operations') implemented faithfully. The
  single invariant under test:

    MetalTankAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Metal Tank Plant Operations
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-category :storage-tank}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :storage-tank (:product-category (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest welding-line-actuate-is-held-and-permanently-blocked
  (testing "a proposal that sets :actuate-welding-line? true -> HOLD, PERMANENT, never reaches request-approval even though the equipment is verified and registered"
    (let [[db actor] (fresh)
      ...
      (is (not= :interrupted (:status res)))
      (is (some #{:welding-line-actuate-blocked} (-> (store/ledger db) last :basis)))
      (is (empty? (store/maintenance-history db))))))

(deftest code-stamp-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-code-stamp? true -> HOLD, PERMANENT, never reaches request-approval -- this actor has no pressure-vessel-certification authority"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:code-stamp-authority-blocked} (-> (store/ledger db) last :basis)))
      (is (not= :process-vessel (:product-category (store/batch db "batch-001"))) "no partial commit -- the whole proposal is blocked"))))
  ...
  (testing "scheduling the SAME maintenance record twice -> HOLD on the second attempt"
    (let [[db actor] (fresh)
          _ (exec-op actor "t9a" {:op :schedule-maintenance :effect :propose :subject "mnt-1"
                                  :value {:equipment-id "weld-001" :maintenance-type :torch-inspection
                                          :scheduled-date "2026-08-01" :actuate-welding-line? false}} coordinator)
          _ (approve! actor "t9a")
...
(deftest invalid-product-category-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:product-category :unobtainium-vessel}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-product-category} (-> (store/ledger db) last :basis)))
    (is (not= :unobtainium-vessel (:product-category (store/batch db "batch-001"))) "fabricated product-category never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "weld-001" :severity :moderate
                                            :description "weld-fume extraction hood underperforming"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "weld-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
