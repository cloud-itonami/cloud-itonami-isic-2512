(ns metaltankmfg.operation
  "MetalTankOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (MetalTankAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Metal Tank Plant
  ...
  (:require [langgraph.graph :as g]
            ...
            [metaltankmfg.advisor :as advisor]
...
(defn build
  "Compiles a MetalTankOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `metaltankmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MetalTankAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Metal Tank Plant Operations Governor -- independent censor.
      (g/add-node :govern
