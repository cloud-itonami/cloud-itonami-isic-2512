(ns metaltankmfg.registry
  "Pure-function domain logic for the metal-tank/reservoir/container
  fabrication shop plant-operations coordination actor -- equipment/
  batch verification, shipment-weight recompute, product-category
  ...
  pre-existing `kotoba-lang/metaltankmfg`-style capability library to
  wrap (verified: no such repo exists). The domain logic therefore
  lives here as pure functions, re-verified INDEPENDENTLY by
  `metaltankmfg.governor` -- the same 'ground truth, not self-report'
  ...
  coordinated shipment), not the act of actuating a welding line or
  pressure-testing line, dispatching a real freight carrier, or issuing
  a pressure-vessel certification (e.g. an ASME code stamp) -- this
  actor NEVER does any of those (see README `What this actor does
  NOT do`).

  SCOPE NOTE: ISIC 2512 (this actor) is the 'tanks, reservoirs and
  containers of metal' class -- a metal-fabrication shop (welding line
  -> pressure-testing line -> forming line) that welds and forms sheet/
  plate metal into storage tanks, reservoirs, metal containers, gas
  cylinders, process vessels and central-heating boilers. This is
  distinct from `cloud-itonami-isic-2513` (Manufacture of steam
  generators, except central heating hot water boilers -- boiler/
  steam-generator equipment upstream of this shop's own tank/reservoir/
  container scope), `cloud-itonami-isic-2591` (Forging, pressing,
  stamping and roll-forming of metal -- a distinct heavy metal-forming
  process, not welded-tank fabrication), and `cloud-itonami-isic-2511`
  (Manufacture of structural metal products -- a distinct structural-
  steel product family). This actor's own hazard profile centers on
  weld-fume exposure, pressure-test rupture risk at the pressure-
  testing line, heavy-plate crush hazard during forming/handling,
  confined-space entry hazard (tank/vessel interior inspection and
  welding), and hot-work fire risk -- not molten-metal handling (2431)
  or stamping-press pinch-point hazard (2599).")

;; ----------------------------- constants -----------------------------

(def valid-product-categories
  "The closed set of product-category values a production-batch (a
  welded/formed lot) record may declare -- the standard 'tanks,
  reservoirs and containers of metal' families this shop produces.
  ...
  through."
  #{:storage-tank :reservoir :metal-container :gas-cylinder
    :process-vessel :central-heating-boiler})

(def valid-process-types
  "The closed set of PRIMARY PROCESS types this shop's own production
  line may take -- welding, fabrication, pressure-testing, rolling, or
  forming. A metal-tank/reservoir/container shop in this class never
  ships a raw casting or a stamped/pressed sheet-metal part (those are
  a different actor's own scope, not this actor's)."
  #{:welding :fabrication :pressure-testing :rolling :forming})
...
(defn product-category-valid?
  ...
  values (storage-tank/reservoir/metal-container/gas-cylinder/process-
  vessel/central-heating-boiler family)? nil/blank is treated as
...
(defn- unsigned-certificate
  ...
  actor's. Crucially, this is NEVER a pressure-vessel certification
  (e.g. ASME code stamp) -- see README `What this actor does NOT do`;
  it is a plain draft-schedule/draft-shipment coordination record."
...
(defn register-maintenance
  ...
  welding-line/pressure-testing-line/forming-line maintenance window
  against a verified, registered piece of equipment. Pure function --
  does not actuate the welding line or pressure-testing/forming line
  or execute any maintenance; it builds the RECORD a shop coordinator
  would keep. `metaltankmfg.governor` independently re-verifies the
  equipment's own verified/registered ground truth, and permanently
  blocks any attempt to directly actuate the welding line (see README
...
(defn register-shipment
  ...
  outbound tank/reservoir/container shipment against a verified,
  ...
  keep. `metaltankmfg.governor` independently re-verifies the
