(ns metaltankmfg.store
  "SSoT for the metal-tank/reservoir/container fabrication shop
  ...
  self-contained (no external tank/reservoir/container-manufacturing
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch (a
                             welded/formed lot)'s product-category/
                             ...
                             in the shop's production ledger;
                             `:shipped-weight-kg` tracks the batch's
                             own cumulative-shipped ground truth.
    - `equipment`         -- a welding-line/pressure-test-rig/forming-
    ...
    - `maintenance`       -- a scheduled welding-line/pressure-test-rig/
                             forming-line-equipment maintenance-window
                             DRAFT against a piece of equipment
                             (`metaltankmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound tank/reservoir/
                             container shipment DRAFT
                             (`metaltankmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (metaltankmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (metaltankmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-category :storage-tank :process-type :welding
                :material "Welded Carbon-Steel Storage Tank Lot"
                :weight-kg 25000.0 :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-weight-kg 5000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-category :metal-container :process-type :forming
                :material "Formed Steel Drum Lot"
                :weight-kg 8000.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-weight-kg 7600.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-category :process-vessel :process-type :pressure-testing
                :material "Pressure-Tested Process Vessel Lot"
                :weight-kg 15000.0 :defect-rate-percent 1.2
                :verified? false :registered? false
                :shipped-weight-kg 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"weld-001" {:id "weld-001" :kind :welding-station
               :verified? true :registered? true
               :last-maintenance-date "2026-05-01"}
   "pretest-002" {:id "pretest-002" :kind :pressure-test-rig
                  :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `metaltankmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `metaltankmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  coordinated against it); one verified+registered welding-station unit
  (schedulable for maintenance), one UNVERIFIED/unregistered
  pressure-test-rig unit (blocks any maintenance scheduling against
