(ns metaltankmfg.advisor
  "MetalTankAdvisor -- the *contained intelligence node* for the
  metal-tank/reservoir/container fabrication shop plant-operations
  coordination actor (ISIC 2512, manufacture of tanks, reservoirs and
  containers of metal).

  It normalizes production-batch patches (product-category/weight/
  defect-rate), drafts a welding-line/pressure-test-rig/forming-line
  maintenance scheduling proposal against a piece of equipment, drafts
  a safety-concern flag, and drafts an outbound tank/reservoir/
  container shipment coordination proposal against a production batch.
  CRITICAL: it is a smart-but-untrusted advisor. It returns a
  *proposal* (with a rationale + the fields it cited), never a
  committed record and NEVER a real welding-line actuation, freight
  dispatch, or pressure-vessel certification (e.g. an ASME code stamp).
  Every output is censored downstream by `metaltankmfg.governor` before
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `metaltankmfg.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; line-control effect and NEVER a
                                 ; pressure-vessel-certification effect
  ...
  `metaltankmfg.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  "Production-batch (welded/formed lot) intake upsert -- the advisor
  only normalizes/validates the patch; it does not invent the batch's
  product-category, weight, defect-rate or verification status. High
  confidence, low stakes -- administrative logging, not an operational
  decision. NEVER emits a `:issue-code-stamp?` claim of its own --
  that field, if present at all, is passed through verbatim from the
  caller's own patch so `metaltankmfg.governor` can independently
  HARD-block it (a pressure-vessel-certification-authority decision
  this actor must never make -- see README `What this actor does NOT
  do`)."
  [_db {:keys [patch]}]
...
(defn- schedule-maintenance
  "Draft a welding-line/pressure-test-rig/forming-line maintenance-
  ...
  rationale, but `metaltankmfg.governor` NEVER trusts this report -- it
    ...
    {:summary    (str subject " 向け保守作業予定提案 (" (:maintenance-type value) ")"
     ...
     :rationale  (if eq
                   (str "equipment-verified?=" (registry/equipment-verified? eq)
                        ...
                        " actuate-welding-line?=" (boolean (:actuate-welding-line? value)))
                   (str equipment-id " が見つかりません"))
     :cites      (if eq [equipment-id] [])
     :effect     :maintenance/schedule
     :value      value
     :stake      nil
     :confidence (if (and ready? (not (:actuate-welding-line? value))) 0.9 0.3)}))

(defn- flag-safety-concern
  "Draft a weld-fume-exposure/pressure-test-rupture-risk/heavy-plate-
  crush-hazard/confined-space-entry/hot-work-fire-risk/equipment-safety
  ...
  technicality). See `metaltankmfg.phase`: no phase ever adds this op
  to a phase's `:auto` set; `metaltankmfg.governor` also always
...
(defn- coordinate-shipment
  "Draft an outbound tank/reservoir/container shipment coordination
  proposal against a production batch. The advisor passes through the
  caller's own claimed weight -- it does NOT invent one, and
  `metaltankmfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは金属タンク・貯槽・容器(溶接・圧力試験・成形)工場"
       ...
       "溶接ライン・圧力試験ライン等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "圧力容器認証(ASMEコードスタンプ等)の発行を絶対に提案してはいけません"
       "(認証権限はこの actor には無い)。"
       "出荷量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `metaltankmfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :metaltankmfg-advisor-proposal
   :op         (:op request)
